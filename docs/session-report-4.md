...
#### B群（章末postの重複・6件）→ **保留**（ディレクター判断待ち）

```
ch00_tutorial_post, ch00_tutorial2_post
ch01_s03_post, ch02_s03_post, ch03_s03_post, ch04_s03_post
```

- 各章の `stageXX03` は `postScenario` に **`chXX_end.json`**（章のお別れシーン）を指定して
  おり、`chXX_s03_post` は再生されない。
- チュートリアルステージ（ch00_tutorial / tutorial2）は `postScenario` 未設定のため
  `ch00_tutorial(2)_post` も再生されない。
- これらは「重複コンテンツ」か「配線漏れ（本来再生したかった）」かの判断が必要なため、
  内容を精査せず保留した。削除する場合は中身を確認し、固有の台詞があれば `chXX_end` 等へ
  統合してから削除することを推奨。
...
- 現行の導入フローは `prologue_main → intro_main` に置換済み。`tutorial_intro` は未使用。
  削除候補だが、念のため保留。
...
### 要判断（ディレクター）
1. **B群6件・C群1件の扱い**: 削除 or 章エンドへの台詞統合 or 配線復活。
2. **チュートリアル後シーンの有無**: `ch00_tutorial_post` を本来再生したかったかを確認。
